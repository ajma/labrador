...
export async function projectRoutes(app: FastifyInstance) {
  ...
  const exposureService = (app as any).exposureService as
    | ExposureService
    | undefined;
  const updateCheckerService = (app as any).updateCheckerService as
  ...
  app.get("/adoptable", async (request, reply) => {
    if (!adoptService)
      return reply.code(503).send({ error: "Docker not available" });
  ...
  app.post<{ Body: { stackNames: string[]; isInfrastructure?: boolean } }>(
    "/adopt",
    async (request, reply) => {
      if (!adoptService)
        return reply.code(503).send({ error: "Docker not available" });
      ...
      if (!Array.isArray(stackNames) || stackNames.length === 0) {
        return reply
          .code(400)
          .send({ error: "stackNames must be a non-empty array" });
      }
      const userId = (request.user as any).id;
    ...
    async (request, reply) => {
      ...
      if (!exposureService) {
        return {
          active: false,
          domain: "",
          message: "Exposure service not available",
        };
      }
      ...
      try {
        ...
        return (
          status || {
            active: false,
            domain: "",
            message: "Exposure not configured",
          }
        );
      } catch (error: any) {
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!updateCheckerService) {
        return reply
          .code(503)
          .send({ error: "Update checker is not available" });
      }
  ...
  app.post("/compose/validate", async (request) => {
    ...
    if (content.length > 102400) {
      return {
        valid: false,
        errors: [{ message: "Content exceeds maximum size (100KB)" }],
        warnings: [],
      };
    }
